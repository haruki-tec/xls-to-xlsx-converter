...
def select_folder_before():
    ...
    global selected_folder_before_path
...
def select_folder_after():
    folder_path = filedialog.askdirectory()  # フォルダ選択ダイアログを表示
    global selected_folder_after_path
    if folder_path:  # フォルダが選択された場合
...
def convert_data():
    if selected_folder_before_path == '' or selected_folder_after_path == "":
        messagebox.showwarning("キャンセル", "どちらかのフォルダが選択されません。")
    else:
        tp.convert_xls_to_xlsx(selected_folder_before_path,selected_folder_after_path)
